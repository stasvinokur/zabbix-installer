...
OUTPUT=$(yes 'N' | sudo DEBIAN_FRONTEND=noninteractive apt-get install -y zabbix-agent2 2>&1)
INSTALL_EXIT_CODE=$?

# Если команда вернулась с ошибкой или в выводе есть строка о том, что нет кандидата — выходим
if [ $INSTALL_EXIT_CODE -ne 0 ] || echo "$OUTPUT" | grep -q "E: Package 'zabbix-agent2' has no installation candidate"; then
  echo "ERROR: Failed to install zabbix-agent2. It might have no installation candidate or another error occurred."
  exit 1
fi
